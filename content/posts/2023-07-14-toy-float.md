...
interval $[1, 2)$. And throw away the top bit, since it's always 1.
In other words, the single alias with $1 \le m < 2$ is the only one that can be
encoded. Other aliases would have bits above the implicit bit or require
the implicit bit itself to be zero.
